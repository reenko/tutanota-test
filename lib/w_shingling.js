class WShingling {
    ...
    _textCanonization(text) {
        ...
        const textWithoutSpecSympols = this._removeUselessSpaces(this._removeAllSpecSymbols(preparedText));
        if (textWithoutSpecSympols === '') {
            // if text contains only spec symbols -> keep this symbols in text
            return preparedText;
        }

        const textWithoutShortWord = this._removeUselessSpaces(this._removeShortWords(textWithoutSpecSympols));
        if (textWithoutShortWord === '') {
            // if text contains only short words -> keep this words in text
            return textWithoutSpecSympols;
        }

        text = this._advanceCanonization(textWithoutShortWord);
